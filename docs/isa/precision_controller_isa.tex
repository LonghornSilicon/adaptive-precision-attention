...
\textbf{Reference}:  & \texttt{LonghornSilicon/adaptive-precision-attention} \\
\end{tabular}\\[1em]
...
\textbf{Bit-exact reference:} \texttt{sw/reference\_model/precision\_controller\_ref.py}
...
The Python model at
\texttt{sw/reference\_model/precision\_controller\_ref.py} is the canonical
...
(see \texttt{sw/reference\_model/test\_precision\_controller\_ref.py}). Any
...
\midrule
0 (now) & This branch                & Bit-accurate Python reference model        & Model + ISA + tests on the \texttt{compiler-integration-prep} branch \\
1       & When ZCU102/104 arrives    & AXI-Lite + AXI-Stream on Zynq Ultrascale+ & Vivado bitstream + PYNQ driver \\
2       & After KV cache + token + memory blocks complete & Same AXI interface, more blocks visible & Multi-block FPGA project \\
