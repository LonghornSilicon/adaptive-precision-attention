...
\textbf{Affil.}:  & UT Austin            & \textbf{Repo}:     & \href{https://github.com/LonghornSilicon/adaptive-precision-attention}{LonghornSilicon/adaptive-precision-attention} \\
\end{tabular}\\[0.4em]
...
A four-block LLM inference accelerator targeting TSMC 16FFC via the TSMC
University Program (tape-out target Q3 / Q4 2026):
\begin{enumerate}
...
\item \textbf{KV Cache Engine} --- on-chip SRAM / eDRAM with compression
  (2--4 bit quantization plus outlier handling). \emph{Not yet implemented.}
...
\item \textbf{Memory Hierarchy Controller} --- L1 SRAM, L2 eDRAM, off-chip
  LPDDR5 with SHIELD-style refresh disable on short-lived data.
...
\texttt{LonghornSilicon/adaptive-precision-attention}. The TSMC 16FFC PDK
