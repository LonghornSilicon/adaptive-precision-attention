...
{\large Block 2 of 4 --- Attention Compute Unit (ACU), MAC engine}\\[2em]
...
\midrule
\texttt{pe\_grid\_m}     & 16           & PE grid rows; area-budget estimate \\
\texttt{pe\_grid\_n}     & 16           & PE grid cols; area-budget estimate \\
\texttt{int8\_throughput}& 256 MACs/cyc & One INT8 MAC per PE \\
\texttt{fp16\_throughput}& 64 MACs/cyc  & FP16 PE is $\sim$$4\times$ INT8 area, so $\frac{1}{4}$ the count \\
...
\texttt{LonghornSilicon/adaptive-precision-attention}.}
