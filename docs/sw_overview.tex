...
\textbf{Repository}:     & \texttt{LonghornSilicon/adaptive-precision-attention} \\
...
\item \textbf{ACU (Attention Compute Unit)} --- decides INT8 vs FP16
      ...
      \end{itemize}
\item \textbf{KV Cache Engine} --- on-chip SRAM / eDRAM with
      compression on writes, decompression on reads.
\item \textbf{Token Importance Unit} --- per-token attention-weight
      accumulator driving mixed-precision retention.
\item \textbf{Memory Hierarchy Controller} --- L1 SRAM / L2 eDRAM /
      off-chip LPDDR5 routing.
\end{enumerate}

For compiler co-design, blocks 1 (and its two sub-components) are
the first to be fully specified in software. Blocks 2--4 follow the
same template as they land.
...
\toprule
Block                       & C++ class                          & C API                 & Python                       & Status \\
\midrule
Precision Controller        & \texttt{lhsi::PrecisionController} & \texttt{lhsi\_pc\_*}  & \texttt{PrecisionController} & 143/143 vs RTL \\
MAC Array                   & \texttt{lhsi::mac::MacArray}       & \texttt{lhsi\_mac\_*} & \texttt{MacArray}            & numpy agreed \\
...
2 & blocks 3/4/5 land        & Same AXI, more blocks visible                & Larger Vivado project \\
...
\item \textbf{Token Importance Unit.} Accumulator + classifier;
      ...
      is decided.
\item \textbf{KV Cache Engine.} Gated on the compression-algorithm
      decision (GEAR / RotateKV / Lexico); $\sim$3--4 weeks once
      decided.
\item \textbf{Memory Hierarchy Controller.} Thin allocator first,
      refine when L1 / L2 / DRAM behavior matters; $\sim$1 week.
\end{enumerate}
...
\texttt{LonghornSilicon/adaptive-precision-attention} and updated
